...
import { ProductBotCommandScope } from "constant";
...
const withCommandScope = (args: any) => {
  const store = commandStore.getStore();
  const commandScopes = productMetadata.listCommands();
  const currentCommandScope =
    commandScopes.find((c) =>
      stringUtils.equal(
        (c?.telegram_command || "").toLowerCase(),
        "/" + (store?.data?.command || "").toLowerCase()
      )
    ) ?? null;

  if (
    currentCommandScope?.scope ===
    ProductBotCommandScope.PRODUCT_BOT_COMMAND_SCOPE_PRIVATE_ONLY
  ) {
    return [store?.data.user_id, ...args.slice(1)];
  }
...
const attachFooter = function (
  ...
  return async function (...args: any) {
    try {
      ...
      const tip = productMetadata.randomTip();
...
Telegram.prototype.sendMessage = async function (...args: any) {
  return attachFooter(this, sendMessage, "send")(...withCommandScope(args));
};
