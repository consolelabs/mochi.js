...
import { mochiApi, payApi } from "adapters";
...
async function getAssets(
  ...
  switch (location) {
    case "offchain":
      ...
      balances = offchainBal.filter((i: any) => Boolean(i));
      pnl = 0;
      break;

    case "onchain":
      const res = await assetFetcher(location)(profileId, address, chainType);
      balances = res.balance?.filter((i: any) => Boolean(i)) ?? [];
      pnl = Number(res.pnl);
      if (Number.isNaN(pnl)) {
        pnl = 0;
      }
  ...
  switch (location) {
    case "offchain":
      return (profileId: string) =>
        payApi.mochiWallet.getOffchainBalances(profileId);
    case "onchain":
      return (profileId: string, address: string, chainType: string) =>
        mochiApi.users.getWalletAssets(profileId, address, chainType);
  }
}
