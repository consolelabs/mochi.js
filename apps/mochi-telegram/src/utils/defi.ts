...
async function parseMoniker(moniker: string) {
  const { data = [] } = await api.base.configDefi.getDefaultMonikers();
...
export function parseSymbol<T extends { amount: number; symbol: string }>() {
  return async function (payload: T) {
    const { ok, data } = await api.pay.tokens.getSupported(payload.symbol);
    if (!ok) throw new Error("Cannot get supported tokens");
    if (!data.length) {
...
>(ctx: Context) {
  return async function (payload: T) {
    ...
    const { ok: okProfile, data: profile } = await api.profile.telegram.getById(
      {
        telegramId: userId?.toString() ?? "",
      }
    );
    if (!okProfile) throw new Error("Cannot get profile of user");
    const { ok: okBal, data: balances } = await api.pay.mochiWallet.getBalance({
      profileId: profile.id,
      token: payload.symbol,
    });
    if (!okBal) throw new Error("Cannot get balance of user");
    ...
    if (!balance) throw new Error("Cannot get balance of user");
    if (
...
>() {
  return async function (payload: T) {
    ...
    if (stringUtils.equal(payload.platform, "discord")) {
      const { ok, data } = await api.profile.discord.getById({
        discordId: payload.target.replace("@", ""),
      });
      if (!ok) {
    ...
    if (stringUtils.equal(payload.platform, "telegram")) {
      const { ok, data } = await api.profile.telegram.getById({
        telegramId: payload.target.replace("@", ""),
      });
      if (!ok) {
